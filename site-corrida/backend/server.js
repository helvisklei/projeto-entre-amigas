...
const {
  submitToGoogleForm,
  getInscriptionCount,
} = require("./google-forms-integration");
...
if (process.env.DATABASE_URL) {
      ...
      if (check.rows[0].count === 0) {
        await db.query(
          ...
          [
            ...
            "2025-05-17",
            "Corrida beneficente para as amigas",
            "Recife - PE",
            true,
          ]
        );
  ...
  console.warn(
    "DATABASE_URL não definido — usando banco em memória (apenas para testes)"
  );
  usingMockDb = true;
...
const verifyJWT = (req, res, next) => {
  ...
  try {
    const decoded = jwt.verify(
      token,
      process.env.JWT_SECRET || "seu-segredo-secreto-aqui"
    );
    req.admin = decoded;
    next();
  } catch (err) {
  ...
  try {
    ...
    if (usuario === DEFAULT_ADMIN_USER && senha === DEFAULT_ADMIN_PASS) {
      ...
      const token = jwt.sign(
        { id: 1, usuario, email: "admin@helvisklei.com" },
        process.env.JWT_SECRET || "seu-segredo-secreto-aqui",
        { expiresIn: "24h" }
      );
    ...
    const result = await db.query(
      "SELECT * FROM admins WHERE usuario = $1 AND ativo = true",
      [usuario]
    );
...
app.post("/inscricao", async (req, res) => {
  try {
    if (!req.body)
      return res.status(400).json({ message: "Corpo da requisição vazio" });
    const { nome, telefone, email, cpf, cidade, tamanho_camisa, autorizado } =
      req.body;
    if (!nome || !telefone || !email) {
      return res
        .status(400)
        .json({ message: "nome, telefone, email são obrigatórios" });
    }
    ...
    if (total >= 100) {
      return res.status(400).json({
        message:
          "Limite de 100 inscrições atingido. Próximo evento: Setembro 2025",
...
app.get("/relatorio/excel", async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="relatorio_inscricoes.xlsx"'
    );
...
app.get("/relatorio/pdf", async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="relatorio_inscricoes.pdf"'
    );
    doc.pipe(res);
    ...
    rows.forEach((r) => {
      doc
        .fontSize(12)
        .text(
          `${r.nome} — ${r.email} — ${r.telefone} — Pago: ${
            r.pago ? "Sim" : "Não"
          }`
        );
    });
...
app.get("/relatorio/pdf", async (req, res) => {
  try {
    ...
    doc
      .fontSize(24)
      .font("Helvetica-Bold")
      .text("🏃 CORRIDA ENTRE AMIGAS", { align: "center" });
    doc
      .fontSize(14)
      .font("Helvetica")
      .text("Relatório de Inscrições 2024", { align: "center" });
    doc.moveDown(0.5);
    ...
    doc
      .fontSize(11)
      .font("Helvetica-Bold")
      .text(`📊 RESUMO EXECUTIVO`, { align: "left" });
    doc
      .fontSize(10)
      .font("Helvetica")
      .text(`Total de Inscritos: ${totalInscritos}`, { indent: 10 });
    ...
    doc.text("Telefone", colX.telefone, doc.y - doc.heightOfString("Nome"), {
      width: 80,
    });
    doc.text("Email", colX.email, doc.y - doc.heightOfString("Nome"), {
      width: 80,
    });
    doc.text("Cidade", colX.cidade, doc.y - doc.heightOfString("Nome"), {
      width: 80,
    });
    doc.text("Pago", colX.pago, doc.y - doc.heightOfString("Nome"), {
      width: 60,
    });
    doc.text("Data", colX.data, doc.y - doc.heightOfString("Nome"), {
      width: 60,
    });
    doc.moveDown(0.7);
    ...
    rows.forEach((r) => {
      ...
      doc.text(
        r.telefone,
        colX.telefone,
        doc.y - doc.heightOfString(r.nome.substring(0, 20)),
        { width: 80 }
      );
      doc.text(
    ...
    doc
      ...
      .font("Helvetica")
      .text(
        `© 2025 HVK PRODUÇÕES. Todos os direitos reservados.`,
...
app.post("/admin/pagamento", async (req, res) => {
  ...
  try {
    await db.query("UPDATE inscricoes SET pago = $1 WHERE id = $2", [
      !!pago,
      id,
    ]);
    res.json({ ok: true });
...
app.get("/admins", async (req, res) => {
  try {
    if (usingMockDb)
      return res.json([
        { id: 1, usuario: "admin", email: "admin@local", ativo: true },
      ]);
    const { rows } = await db.query(
      "SELECT id, usuario, email, ativo, created_at FROM admins ORDER BY created_at DESC"
    );
    res.json(rows);
  } catch (err) {
...
app.post("/admins", async (req, res) => {
  try {
    ...
    if (usingMockDb)
      return res.status(400).json({ message: "Não suportado em modo mock" });
...
app.put("/admins/:id", async (req, res) => {
  try {
    ...
    if (usuario) {
      await db.query(
        "UPDATE admins SET usuario = $1, updated_at = NOW() WHERE id = $2",
        [usuario, id]
      );
    }
    if (email) {
    ...
    if (senha) {
      await db.query(
        "UPDATE admins SET senha = $1, updated_at = NOW() WHERE id = $2",
        [senha, id]
      );
    }
...
app.put("/admins/:id/toggle", async (req, res) => {
  try {
    ...
    await db.query(
      "UPDATE admins SET ativo = $1, updated_at = NOW() WHERE id = $2",
      [!!ativo, id]
    );
...
app.get("/events", async (req, res) => {
  try {
    ...
    const { rows } = await db.query(
      "SELECT * FROM events WHERE ativo = true ORDER BY data_evento DESC"
    );
    res.json(rows);
  } catch (err) {
...
app.post("/events", verifyJWT, async (req, res) => {
  try {
    ...
    if (!titulo || !data_evento)
      return res
        .status(400)
        .json({ message: "Título e data são obrigatórios" });
...
app.get("/testimonials/:eventId", async (req, res) => {
  try {
      ...
      return res.json([
        {
          ...
          depoimento: "Que privilégio correr entre amigas!",
          posicao: 1,
          genero: "F",
        },
      ]);
    ...
    const { rows } = await db.query(
      "SELECT * FROM testimonials WHERE event_id = $1 ORDER BY posicao ASC",
      [eventId]
    );
    res.json(rows);
  } catch (err) {
    console.error(
      "Erro em GET /testimonials/:eventId:",
      err.message,
      err.stack
    );
...
app.post("/testimonials", verifyJWT, async (req, res) => {
  try {
    ...
    if (!event_id || !nome || !depoimento)
      return res
        .status(400)
        .json({ message: "Campos obrigatórios: event_id, nome, depoimento" });
